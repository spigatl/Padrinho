    ...
    foreach($_POST AS $key => $value)
    {
        ...
        {   
            if (!ereg("^[a-z,A-Z,0-9,_,@,&,$,á-ú,ç,Ç,â-û,(,),[:space:],.,-]+$", $value)) {
                $errors[1] = "<p class=p_erro><h12>Essses caracteres não sao validos: &#33; &#34; &#35; &#37; &#39; &#42; &#43; &#44; &#47;</h12></p><br>";
                red($key);
            }
            $field = $key;
            $key = $value;
            ...
            if(in_array($field, $requiredFields) && $field == 'fr_nome')
            {
                ...
                if ($result) {
                    while($row = mySQL_fetch_array($result)) {
                        $errors[] = "<p class=p_erro><h12>Email <b> $row[E_EMPRESA] </b> ja em uso.</h12></p><br>";
                        red($field);    
                    }
                } 
...
else
{
    ...
    // nome da img que ira para banco
    ...
    $insert = "INSERT INTO empresa (E_EMPRESA, E_COMENTARIO, E_IMG, E_RUA, E_NUMERO, E_UF, E_TEL, E_MAIL, E_SITE, E_CIDADE, E_CATEGORIA, E_ID, E_CNPJ, E_CEP, E_BAIRRO) VALUES ('$fr_nome', '$fr_coment', '$fr_img', '$fr_end', '$fr_num', '$fr_uf', '$fr_tel', '$fr_mail', '$fr_site', '$fr_cidade', '$fr_ctg', '$fr_id', '$fr_cpf', '$fr_cep', '$fr_bairro')"; 
    } else {
    $insert = "INSERT INTO empresa (E_EMPRESA, E_COMENTARIO, E_RUA, E_NUMERO, E_UF, E_TEL, E_MAIL, E_SITE, E_CIDADE, E_CATEGORIA, E_ID, E_CNPJ, E_CEP, E_BAIRRO) VALUES ('$fr_nome', '$fr_coment', '$fr_end', '$fr_num', '$fr_uf', '$fr_tel', '$fr_mail', '$fr_site', '$fr_cidade', '$fr_ctg', '$fr_id', '$fr_cpf', '$fr_cep', '$fr_bairro')";    
    }
...
<form id="formElem" name="formElem" action="" onsubmit="" method="post" enctype="multipart/form-data">
    <div id="fr_error">
        <?php
        if (isset($_POST['cadastrar'])) {
            echo "$errorString";
    ...
    <div class="row" style="width: 65%; float: left">
        ...
        <input type="checkbox" name="fr_tal" id="ex04" class="{labelOn: 'Opção 2', labelOff: 'Opção 1'}" />
        <p>
...
for ($i = 0; $i < 19; $i++) {
    if (isset($_POST[$i])) {
        echo "<li>
        <input id=$i name=$i type=\"checkbox\" checked=\"yes\" value=$i />
        <label for=$i>$ctg[$i]</label>
        </li>";
    } else {
        echo "<li>
...
<div id="photodiv" style="width: 45%; float: left">
    ...
    <p class="perfildireita">
        ...
        <input id="fr_tel" name="fr_tel"  type="text" value='<?php if (isset($_POST['cadastrar'])) { echo "$fr_tel"; } ?>' AUTOCOMPLETE=OFF/>
    </p>
    <p class="perfildireita">
        <label for="fr_cpf">CPF/CNPJ:</label>
        <input id="fr_cpf" name="fr_cpf"  type="text" value='<?php if (isset($_POST['cadastrar'])) { echo "$fr_cpf"; } ?>' AUTOCOMPLETE=OFF/>
    </p>
    <p class="perfildireita">
